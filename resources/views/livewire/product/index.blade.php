        ...
        @foreach($entries as $key_entry => $entry)
            <div class="flex flex-col rounded-lg shadow-lg overflow-hidden {{ ($key_entry+1) !== count($entries) ? 'mb-8' : '' }}">
                <div class="flex-shrink-0">
                    <a href="{{ route('products.show.slug',['date'=>date('Y-m-d',strtotime($entry->created_at)),'slug'=>$entry->slug]) }}" class="">
                ...
                <div class="flex-1 bg-white dark:bg-gray-900 p-6 flex flex-col justify-between">
                    <div class="flex-1">
                        ...
                        <a href="{{ route('products.show.slug',['date'=>date('Y-m-d',strtotime($entry->created_at)),'slug'=>$entry->slug]) }}" class="block mt-2">
